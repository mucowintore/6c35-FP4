<script>
  ...
  function draw() {
    ...
    const m = { top: 46, right: 66, bottom: 42, left: 58 };
    ...
    g.append('g').attr('transform', `translate(0,${h})`)
      ...
      .call(g => g.selectAll('text').attr('fill', '#9C9890')
        .style('font-size', '12px').style('font-weight', '600').style('font-family', font))
      .call(g =>
    ...
    g.append('g')
      ...
      .call(g => g.selectAll('.tick line').clone()
        .attr('x2', w).attr('stroke', '#E8E3D9').attr('stroke-width', 0.8))
      .call(g => g.selectAll('text').attr('fill', '#9C9890')
        .style('font-size', '12px').style('font-weight', '600').style('font-family', font));
    ...
    g.append('line')
      ...
      .attr('stroke', '#A79F8E').attr('stroke-dasharray', '4 3');
    g.append('text').attr('x', x(2008)).attr('y', -8).attr('text-anchor', 'middle')
      .attr('fill', '#8C8577').style('font-size', '11px').style('font-weight', '700')
      .style('font-family', font).text('2008');
    ...
    animateLine(lineTop, '#7EA6C4', 2.4, 220);
    animateLine(lineAll, '#173552', 3.4, 0);
    ...
    g.append('text').attr('x', x(2022) + 6).attr('y', y(last.investor_share) + 4)
      .attr('fill', '#173552').style('font-size', '13px').style('font-weight', '700')
    ...
    g.append('text').attr('x', x(2022) + 6).attr('y', y(last.top_decile_share) + 4)
      .attr('fill', '#7EA6C4').style('font-size', '13px').style('font-weight', '700')
    ...
    g.append('text').attr('x', x(2000) - 4).attr('y', y(first.investor_share) + 4)
      .attr('text-anchor', 'end').attr('fill', '#173552')
      .style('font-size', '11px').style('font-weight', '700').style('font-family', mono)
      .text(Math.round(first.investor_share * 100) + '%');

    // legend
    const lg = svg.append('g').attr('transform', `translate(${m.left + 8}, 16)`);
    lg.append('line').attr('x1', 0).attr('x2', 18).attr('y1', 0).attr('y2', 0)
      .attr('stroke', '#173552').attr('stroke-width', 3.4);
    lg.append('text').attr('x', 24).attr('y', 4).attr('fill', '#46433C')
      .style('font-size', '12px').style('font-weight', '600').style('font-family', font).text('All sales');
    lg.append('line').attr('x1', 95).attr('x2', 113).attr('y1', 0).attr('y2', 0)
      .attr('stroke', '#7EA6C4').attr('stroke-width', 2.4);
    lg.append('text').attr('x', 119).attr('y', 4).attr('fill', '#46433C')
      .style('font-size', '12px').style('font-weight', '600').style('font-family', font).text('Top price decile');
  }
</script>
