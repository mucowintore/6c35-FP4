<script>
   ...
   *
   * Two lines diverge from a common floor at zero. Holding climbs into
   * positive territory and stays there. Flipping plunges in 2008-2009
   * to roughly minus 25%, recovers slowly, and ends modestly above
   * zero. The space between them is the visual signature of two
   * opposite investor strategies in two different parts of the city.
   *
   * Both lines play a one-shot timed draw when the section becomes
   * active. Three thousand five hundred milliseconds, eased. A clip
   * rect underneath unzips the gap fill from left to right in lockstep
   * with the lines. End callouts and the 2008 trough callout fade in
   * after both lines settle.
   *
   * The pre-2008 portions of each line render muted, the post-2008
   * portions full strength, the same convention as Section 01. */
  ...
  function runDrawAnimation() {
    ...
    /* Gap fill clip rect grows in lockstep with the lines so the
     * wedge unzips left to right. */
    if (gapClipRect) {
     ...
     * That negative number is the visual lede of this entire chart,
     * so it deserves to bloom in mid-draw rather than waiting until
     * the end. */
    if (troughCallout) {
  ...
  function buildChart() {
    ...
    /* Vertical gradient for the gap fill. Navy at top edge, amber at
     * bottom edge. Slightly more present than round one (peak 14% vs
     * 10%) so the wedge reads as content, not background. */
    ...
    /* Zero baseline. The line above it is overpaying. The line below
     * is buying the crisis. */
    ...
    /* 2008 marker line plus the vertical label along it. */
     ...
     * The boundary year sits in both halves so the lines stay
    ...
    /* End-of-line callouts. Large serif numbers, year in mono caps. */
     ...
     * this entire chart. A small leader line points to the data point. */
    ...
    if (trough) {
      ...
      troughCallout.append('line')
        .attr('x1', tx).attr('y1', ty + 6)
        .attr('x2', tx + 18).attr('y2', ty + 26)
        .attr('stroke', COLOR_AMBER).attr('stroke-width', 1)
        .attr('stroke-opacity', 0.6);
      troughCallout.append('text')
        .attr('x', tx + 22).attr('y', ty + 32)
        .attr('fill', COLOR_END)
        .style('font-family', '"DM Serif Display", Georgia, serif')
        .style('font-size', '20px')
        .style('font-weight', '400')
        .text(Math.round(trough.premium) + '%');
      troughCallout.append('text')
        .attr('x', tx + 22).attr('y', ty + 47)
        .attr('fill', AXIS_TEXT)
        .style('font-family', mono)
        .style('font-size', '9.5px')
        .style('letter-spacing', '0.1em')
        .text('FLIPPING, ' + trough.year);
    }
